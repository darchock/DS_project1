...
public class AVLTree implements Iterable<Integer> {
    ...
    protected LinkedList<Object> list4back = new LinkedList<Object>();

    /*
     * IMPORTANT: You may add code to both "insert" and "insertNode" functions.
     */
	public void insert(int value) {
		list4back.addFirst(true); //adding boolean flag to indicate end of actions related to last insertion
		root = insertNode(root, value);
		if (root.value == value) {
			list4back.addFirst(root);
			list4back.addFirst(ImbalanceCases.NO_IMBALANCE);
		}
	...
	protected Node insertNode(Node node, int value) {
        ...
        if (node == null) { //current insertion is the first insertion in tree
        ...
        //left heavy cases
        if (balance > 1) {
            if (value > node.left.value) { //left-right rotation
            	list4back.addFirst(node.left.right);
            	list4back.addFirst(ImbalanceCases.LEFT_RIGHT);
                node.left = rotateLeft(node.left);
                node = rotateRight(node);
            }
            else { //left-left heavy case - need to perform right-right rotation
            	list4back.addFirst(node.left.left);
            	list4back.addFirst(ImbalanceCases.RIGHT_RIGHT);
            	node = rotateRight(node); //performing the right-right rotation
            }
        }
        else if (balance < -1) { //right-heavy cases
            if (value < node.right.value) { //right-left rotation
            	list4back.addFirst(node.right.left);
            	list4back.addFirst(ImbalanceCases.RIGHT_LEFT);
                node.right = rotateRight(node.right);
                node = rotateLeft(node);
            }
            else { //left-left rotation
            	list4back.addFirst(node.right.right);
            	list4back.addFirst(ImbalanceCases.LEFT_LEFT);
            	node = rotateLeft(node); //performing the left-left rotation
            }
        }
        
        //not insertion of root - in current iteration of current insertion action there was no rotation
        //adding the inserted node to our list.
        
        else if (list4back.peekFirst().equals(true)) {
        	
        	if (node.left != null && node.left.value == value) 
        		list4back.addFirst(node.left);
        	else if (node.right != null && node.right.value == value)
        		list4back.addFirst(node.right);
