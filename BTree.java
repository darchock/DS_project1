...
public class BTree<T extends Comparable<T>> {
    ...
    //Double-ended queue for backtracking method purposes
    protected LinkedList<Object> list4back = new LinkedList<Object>();
//    protected Deque<Object> deqForBack = new ArrayDeque<Object>();
    /**
    ...
    public void insert(T value) {
    	...
    	//we want to keep a boolean flag to indicates if a split occured in last insertion
    	boolean wasSplit = false;
        if (root == null) {
            root = new Node<T>(null, maxDegree);
            root.addKey(value);
            //when it was first insert to the BTree structure
            insertList.addFirst(value);
        } else {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                	//we want to save the median Value returned so we can merge him back in case of backtrack
                	T medianVal = split(currentNode);
                	wasSplit = true;
                    insertList.addFirst(medianVal);
                ...
                if (currentNode.isLeaf()) {
                	...
                	//we added the value to it's place in the leaf node
                	insertList.addFirst(value);
                	wasAdded = true;
                } else {
                    int idx = currentNode.getValuePosition(value);
                ...
                if (wasSplit) {
                	//inserts boolean value to indicate whether was split
                	insertList.addFirst(wasSplit);
                }
            }
        }

        ++size;
        list4back.addFirst(insertList);
    }
